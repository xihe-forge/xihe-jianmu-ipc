...
export function getMessages(opts = {}) {
  if (opts.from && opts.to) {
    return queryBetween.all({ from: opts.from, to: opts.to, limit: opts.limit || 50 });
  }
  if (opts.peer) {
    return queryMessages.all({ peer: opts.peer, limit: opts.limit || 50 });
  }
  return queryRecent.all({ limit: opts.limit || 50 });
}
