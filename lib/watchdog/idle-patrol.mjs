...
export const DEFAULT_IDLE_PATROL_INTERVAL_MS = 30 * 60 * 1000;
...
export const DEFAULT_ACK_DEDUP_MS = 60 * 60 * 1000;
...
const HOLD_PATTERN = /(?:\bhold\b|\bon hold\b|\bhold for ack\b|\bpending\b|\bblocked\b|\bblocker\b|\bwaiting\b|等老板拍|等用户决策|待.+完成|等待|暂停|等)/iu;
...
function isIdlePatrolNudgeText(text) {
  return /^\s*\[idle-patrol(?:\s|])/i.test(String(text ?? ''));
}
...
function getAckDedupPath(stateDir) {
  return join(stateDir, 'ack-dedup-state.json');
}
...
function loadAckDedupState(stateDir) {
  const state = loadJsonFile(getAckDedupPath(stateDir));
  return state && typeof state === 'object' && !Array.isArray(state) ? state : {};
}

function saveAckDedupState(stateDir, state) {
  saveJsonFile(getAckDedupPath(stateDir), state);
}

function ackDedupKey(source, topic) {
  return `${String(source ?? 'unknown')}:${String(topic ?? 'unknown')}`;
}

export function getAckDedupLastAt(stateDir, source, topic) {
  const state = loadAckDedupState(stateDir);
  const value = state[ackDedupKey(source, topic)];
  const timestamp = typeof value === 'object' && value !== null ? value.last_nudge_at : value;
  return Number(timestamp) || 0;
}

export function recordAckDedup(stateDir, source, topic, nowMs) {
  const state = loadAckDedupState(stateDir);
  state[ackDedupKey(source, topic)] = { last_nudge_at: nowMs };
  saveAckDedupState(stateDir, state);
}
...
export function isHoldTask(task) {
  const status = String(task?.status ?? '').toLowerCase();
  if (status === 'hold') return true;
  if (task?.metadata?.hold_reason) return true;
  const text = [
    task?.subject,
    task?.description,
    task?.activeForm,
...
export function analyzeTranscript(session, options = {}) {
  ...
  entries.forEach((entry, index) => {
    ...
    if (entry.type === 'user' || entry.message?.role === 'user') {
      ...
      if (text === '') return;
      if (isIdlePatrolNudgeText(text)) return;
      lastUserIndex = index;
      lastUserText = text;
      lastUserAt = ts;
    }
...
function isGitTimeoutError(error) {
  return error?.code === 'ETIMEDOUT'
    ...
    || /timed out|timeout|ETIMEDOUT/i.test(String(error?.message ?? ''));
}

export function countRecentCommits(cwd, sinceMs, execFileSyncImpl = execFileSync, pathspec = null, options = {}) {
  ...
  const timeout = Number(options.gitTimeoutMs ?? DEFAULT_GIT_TIMEOUT_MS) || DEFAULT_GIT_TIMEOUT_MS;
  try {
    const stdout = execFileSyncImpl('git', args, { cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], timeout });
    ...
    if (isGitTimeoutError(error)) {
      options.stderr?.(`[idle-patrol] warning: git log timed out after ${timeout}ms cwd=${cwd}; skipping session`);
      return { count: 0, lastAt: 0, timedOut: true };
    }
...
function debugDedupDecision(options, detail) {
  if (options.debugDedup !== true && process.env.WATCHDOG_IDLE_PATROL_DEBUG_DEDUP !== '1') return;
  options.stderr(
    `[idle-patrol] dedup decision session=${detail.sessionName} dedupKey=${detail.dedupKey} ` +
    `lastNudgeAt=${detail.lastNudgeAt} nowMs=${detail.nowMs} windowMs=${detail.windowMs} shouldSkip=${detail.shouldSkip} ` +
    `currentLevel=${detail.currentLevel} nextLevel=${detail.nextLevel}`,
  );
}
...
export async function evaluateIdlePatrolSession(session, options = {}) {
  ...
  if (tasks.some((task) => options.isHoldTask(task))) {
    resetEscalationState(stateDir, sessionName, '', nowMs);
    return { session: sessionName, action: 'skip', reason: 'hold-task' };
  }
  ...
  const commits = gitPathspec
    ? options.countRecentCommits(session.cwd, sinceMs, options.execFileSync, gitPathspec, options)
    : { count: 0, lastAt: 0 };
  if (commits.timedOut === true) {
    return { session: sessionName, action: 'skip', reason: 'git-timeout' };
  }
  ...
  const stateDedupKey = `${sessionName}:idle-patrol:state`;
  let nextLevel = 0;
  ...
  const topic = 'idle-patrol';
  const ackDedupLastAt = options.getAckDedupLastAt(stateDir, sessionName, topic);
  const ackShouldSkip = ackDedupLastAt > 0 && nowMs - ackDedupLastAt < options.ackDedupMs;
  debugDedupDecision(options, {
    sessionName,
    dedupKey: ackDedupKey(sessionName, topic),
    lastNudgeAt: ackDedupLastAt,
    nowMs,
    windowMs: options.ackDedupMs,
    shouldSkip: ackShouldSkip,
    currentLevel,
    nextLevel,
  });
  if (ackShouldSkip) {
    return { session: sessionName, action: 'skip', reason: 'ack-dedup' };
  }

  const stateShouldSkip = nextLevel === 0 || (currentLevel === 1 && nowMs - lastNudgeAt < options.l1DedupMs);
  debugDedupDecision(options, {
    sessionName,
    dedupKey: stateDedupKey,
    lastNudgeAt,
    nowMs,
    windowMs: currentLevel === 1 ? options.l1DedupMs : 0,
    shouldSkip: stateShouldSkip,
    currentLevel,
    nextLevel,
  });
  if (stateShouldSkip) {
  ...
  await sendIpcMessage({ to, topic, content }, options);
  if (nextLevel === 2) {
    await sendIpcMessage({ to: options.harnessSessionName, topic, content }, options);
  }
  options.recordAckDedup(stateDir, sessionName, topic, nowMs);
...
export function createIdlePatrol(userOptions = {}) {
  const options = {
    ...
    ackDedupMs: DEFAULT_ACK_DEDUP_MS,
    ...
    debugDedup: process.env.WATCHDOG_IDLE_PATROL_DEBUG_DEDUP === '1',
