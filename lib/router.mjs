...
export function createRouter(ctx) {
  ...
  function flushInbox(session) {
    ...
    const merged = [...persistedMessages, ...recentMessages, ...session.inbox]
      .map((item, index) => {
  ...
  return { routeMessage, send, broadcast, broadcastToTopic, pushInbox, flushInbox, scheduleInboxCleanup };
}
