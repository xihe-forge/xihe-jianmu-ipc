...
import { estimateContextPctFromTranscript } from './context-usage-auto-handover.mjs';
import { createMessage } from './protocol.mjs';
import {
  getCostSummary as getCostSummaryDefault,
  getTokenStatus as getTokenStatusDefault,
} from './ccusage-adapter.mjs';

// 14 个 MCP 工具定义。保持与现有行为和 schema 一致。
export const MCP_TOOL_DEFINITIONS = [
  ...
  },
  {
    name: 'ipc_spawn',
    description:
      'Spawn a new Claude Code or Codex session. Background mode runs a one-shot task and reports back via IPC. Interactive mode opens a new terminal window.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        runtime: {
          ...
          enum: ['claude', 'codex'],
          default: 'claude',
          description: 'Runtime to launch: claude (default) or codex.',
        },
        host: {
          type: 'string',
          enum: ['wt', 'vscode-terminal', 'external', 'vscode-uri'],
          default: 'external',
          description:
            'Spawn host: wt=Windows Terminal new tab / vscode-terminal=VSCode terminal / external=caller handles / vscode-uri=VSCode 内一键 spawn 通过 URI handler',
        },
        cwd: {
          type: 'string',
          description:
  ...
  {
    name: 'ipc_reclaim_my_name',
    description:
      'Reclaim a zombie session slot so this process can connect as `name`. Probes the existing holder with a 5s ping; evicts if no pong. Call before WS connect when cold-start sees a stale holder of your intended name.',
    inputSchema: {
      type: 'object',
      properties: {
        name: {
          type: 'string',
          description: 'Session name to reclaim (typically process.env.IPC_NAME).',
        },
      },
      required: ['name'],
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        status: {
          type: 'string',
          enum: ['pending', 'started', 'in_progress', 'completed', 'failed', 'cancelled'],
  ...
  {
    name: 'estimateContextPct',
    description:
      'Estimate current Claude context usage percentage from transcript token usage. Returns 0-100.',
    inputSchema: {
      type: 'object',
      properties: {
        transcriptPath: {
          type: 'string',
          description: 'Optional transcript JSONL path.',
        },
        contextWindow: {
          type: 'number',
          description: 'Context window size. Default 200000.',
        },
      },
    },
  },
  {
    name: 'ipc_cost_summary',
    description:
      'Return ccusage cost summary for today, 7d, 30d, or all, optionally grouped by IPC/project name or model, with optional hourly buckets.',
    inputSchema: {
      type: 'object',
      properties: {
        window: {
        ...
        group_by: {
          type: 'string',
          enum: ['none', 'ipc_name', 'model'],
          default: 'none',
          description: 'Grouping dimension for returned rows.',
        },
        granularity: {
          type: 'string',
          enum: ['hour', 'day'],
          description: 'Optional aggregation grain. Defaults to hour for today and day otherwise.',
        },
      },
    },
  },
  {
    name: 'ipc_token_status',
    description: 'Return current ccusage 5h block quota status.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
...
function isValidSpawnHost(host) {
  return host === 'wt' || host === 'vscode-terminal' || host === 'external' || host === 'vscode-uri';
}
...
export function createMcpTools(ctx) {
  ...
  const terminateWs =
    typeof ctx.terminateWs === 'function' ? ctx.terminateWs : disconnectWs;
  ...
  const estimateContextPct =
    typeof ctx.estimateContextPct === 'function'
      ? ctx.estimateContextPct
      : (args = {}) => estimateContextPctFromTranscript(args?.transcriptPath, args);
  const getCostSummary =
    typeof ctx.getCostSummary === 'function' ? ctx.getCostSummary : getCostSummaryDefault;
  const getTokenStatus =
    typeof ctx.getTokenStatus === 'function' ? ctx.getTokenStatus : getTokenStatusDefault;
  ...
  async function handleToolCall(name, args) {
    ...
    // ipc_spawn
    if (name === 'ipc_spawn') {
      const { name: sessionName, task, interactive, model, runtime, host, cwd } = args ?? {};
      if (!sessionName || !task) {
      ...
      if (host !== undefined && !isValidSpawnHost(host)) {
        return toTextResult(
          `Invalid host "${host}": must be one of wt, vscode-terminal, external, vscode-uri`,
          true,
        );
      }

      if (runtime !== undefined && !['claude', 'codex'].includes(runtime)) {
        return toTextResult(
          `Invalid runtime "${runtime}": must be one of claude, codex`,
          true,
        );
      }
    ...
    if (name === 'ipc_rename') {
      ...
      terminateWs();
      await Promise.resolve(reconnect());
    ...
    // ipc_reclaim_my_name
    if (name === 'ipc_reclaim_my_name') {
      if (!args?.name || typeof args.name !== 'string') {
        return toTextResult('ipc_reclaim_my_name requires "name"', true);
      }

      try {
        const response = await fetch(`http://${getHubHost()}:${getHubPort()}/reclaim-name`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: args.name }),
        });
        return toTextResult(JSON.stringify(await response.json()));
      } catch (err) {
        return toTextResult(`Failed to reclaim session name: ${getErrorMessage(err)}`, true);
      }
    }
    ...
    if (name === 'estimateContextPct') {
      try {
        const pct = await estimateContextPct(args ?? {});
        return toJsonResult({ contextUsagePct: pct });
      } catch (err) {
        return toTextResult(`Failed to estimate context usage: ${getErrorMessage(err)}`, true);
      }
    }

    if (name === 'ipc_cost_summary') {
      ...
      if (!['today', '7d', '30d', 'all'].includes(window)) {
        return toJsonResult({ ok: false, error: 'window must be one of today, 7d, 30d, all' }, true);
      }
      if (!['none', 'ipc_name', 'model'].includes(groupBy)) {
        return toJsonResult({ ok: false, error: 'group_by must be one of none, ipc_name, model' }, true);
      }
      if (granularity !== undefined && !['hour', 'day'].includes(granularity)) {
        return toJsonResult({ ok: false, error: 'granularity must be one of hour, day' }, true);
      }
      try {
        return toJsonResult(await getCostSummary({ window, group_by: groupBy, granularity }));
      } catch (err) {
        return toJsonResult({ ok: false, error: getErrorMessage(err) }, true);
      }
    }

    if (name === 'ipc_token_status') {
      try {
        return toJsonResult(await getTokenStatus({}));
      } catch (err) {
        return toJsonResult({ ok: false, error: getErrorMessage(err) }, true);
      }
    }
