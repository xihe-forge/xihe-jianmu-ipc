 ...
 * @param {string} taskId - Task ID from Hub POST /task
 * @param {boolean} [online=true] - Whether the agent was online (sent) or buffered (pending)
 * @returns {object} Feishu card JSON
 */
export function buildDispatchCard(target, content, taskId, online = true) {
  const preview = truncate(content);
  const statusNote = online ? '已送达' : '已缓冲（Agent离线）';
...
/**
 * Build a card showing recent message history.
 *
 * @param {Array<{ from: string, to: string, content: string, ts: string|number }>} messages
 * @param {{ peer?: string, total: number }} meta
 * @returns {object} Feishu card JSON
 */
export function buildHistoryCard(messages, meta) {
  const { peer, total } = meta;
  const title = peer ? `📜 消息记录 — ${peer}` : '📜 消息记录';
  ...
  if (messages.length === 0) {
    ...
    const lines = messages.map(m => {
      ...
      const preview = truncate(m.content, 30);
...
// ---------------------------------------------------------------------------
// 8. sessions cleanup cards
// ---------------------------------------------------------------------------

/**
 * Build a card showing sessions_history rows.
 *
 * @param {Array<{ sessionId: string, name: string, spawnAt?: number, endedAt?: number|null, lastSeenAt?: number|null }>} sessions
 * @param {{ total?: number }} meta
 * @returns {object} Feishu card JSON
 */
export function buildSessionsListCard(sessions, meta = {}) {
  const rows = Array.isArray(sessions) ? [...sessions] : [];
  rows.sort((a, b) => (b.spawnAt ?? 0) - (a.spawnAt ?? 0));

  const bodyMd = rows.length === 0
    ? '_暂无 sessions_history 记录_'
    : rows.slice(0, 20).map((row) => {
        const time = row.spawnAt ? new Date(row.spawnAt).toLocaleString('zh-CN') : '-';
        return `**${row.name}** | \`${truncate(row.sessionId, 18)}\` | ${time}`;
      }).join('\n');

  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: `Session 列表 (${meta.total ?? rows.length})` },
      template: 'blue',
    },
    elements: [
      { tag: 'div', text: { tag: 'lark_md', content: bodyMd } },
...
/**
 * Build a cleanup approval card. Confirm button is the only path that executes deletion.
 *
 * @param {{ name?: string, olderThanDays?: number, requestedBy?: string }} request
 * @returns {object} Feishu card JSON
 */
export function buildSessionsCleanupApprovalCard(request = {}) {
  const target = request.name
    ? `IPC name: **${request.name}**`
    : `ended_at 早于 **${request.olderThanDays} 天前**`;
  const bodyMd = `高危删除请求等待 jianmu-pm 审批\n\n${target}\n\n确认后才会执行 sessions_history cleanup。`;

  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: 'Session 清理审批' },
      template: 'orange',
    },
    elements: [
      { tag: 'div', text: { tag: 'lark_md', content: bodyMd } },
      {
        ...
        actions: [
          {
            tag: 'button',
            text: { tag: 'plain_text', content: '确认清理' },
            type: 'danger',
            value: { action: 'cleanup_sessions_confirm', name: request.name ?? null, olderThanDays: request.olderThanDays ?? null },
          },
          {
            tag: 'button',
            text: { tag: 'plain_text', content: '取消' },
            value: { action: 'cleanup_sessions_cancel' },
          },
        ],
      },
    ],
