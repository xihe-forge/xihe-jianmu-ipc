...
export function createRouter(ctx) {
  ...
  function broadcastToTopic(topic, payload, exceptName = null) {
    ...
    for (const [, session] of sessions) {
      ...
      if (session.ws && session.ws.readyState === session.ws.OPEN) {
        push(session, payload, 'broadcast-topic');
      } else {
  ...
  function routeMessage(msg, senderSession) {
    ...
    const deferPersistenceForPendingRebind = msg.type === 'message' && pendingDirectTarget && !topic;
