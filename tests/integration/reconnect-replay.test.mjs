...
test('reconnect replay: 在线期间发送的消息在崩溃后可从 messages 表回放', { timeout: TEST_TIMEOUT }, async () => {
  ...
  try {
    ...
    try {
      const inbox = await waitForWebSocketMessage(
        reconnected,
        (message) => message.type === 'inbox' && Array.isArray(message.messages) && message.messages.length >= sent.length,
      );

      const replayed = inbox.messages.filter((message) => sent.some((item) => item.id === message.id));
      assert.deepEqual(replayed.map((message) => message.id), sent.map((item) => item.id));
      assert.deepEqual(replayed.map((message) => message.content), sent.map((item) => item.content));
    } finally {
      await closeWebSocket(reconnected);
    }
  } finally {
