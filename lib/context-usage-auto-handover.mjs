...
export function isGitTreeClean(cwd = process.cwd()) {
  try {
    // --untracked=no: 只检查 modified/staged·忽略 untracked drafts·不阻 atomic handoff
    const output = execFileSync('git', ['status', '--porcelain', '--untracked=no'], {
      cwd,
      encoding: 'utf8',
  ...
  return {
    async tick(...args) {
      ...
      if (pct === null || pct === undefined || !Number.isFinite(Number(pct))) {
        return { skipped: 'pct-unknown', pct: null };
      }
...
export function createAtomicHandoverTrigger({
  ...
  return async function triggerAtomicHandover() {
    ...
    const quitNotifyResult = await notifyOldSessionQuit({ name: originalName, oldName });
    try {
      lineage?.record?.({
      ...
      stderr(`[context-usage-auto-handover] sessions_history/lineage record skipped: ${errorMessage(error)}`);
    }
    let spawnResult;
    try {
    ...
    return { handoverPath, renamedTo: oldName, renameResult, quitNotifyResult, spawnResult };
  };
}
