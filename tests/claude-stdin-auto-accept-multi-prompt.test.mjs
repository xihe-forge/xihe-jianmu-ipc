...
before(async () => {
  tempDir = await mkdtemp(join(tmpdir(), 'claude-stdin-auto-accept-multi-prompt-'));
...
function trustPromptText(path = 'C:\\Users\\jolen') {
  return [
    'Accessing workspace:',
    path,
...
function developmentChannelPromptText() {
  return [
...
describe('ADR-014 Phase 2 K.O stdin auto-accept multi-prompt state machine', () => {
  test('trust prompt then development-channel prompt each receive one Enter in order', async () => {
    const result = await runWrapper({
      env: {
        CLAUDE_STDIN_AUTO_ACCEPT_EARLY_MS: '5000',
        PTY_MOCK_EXIT_MS: '600',
        PTY_MOCK_DATA_EVENTS: JSON.stringify([
          { delayMs: 10, data: trustPromptText() },
          { delayMs: 200, data: developmentChannelPromptText() },
          { delayMs: 420, data: 'Listening for channel messages from: server:ipc' },
        ]),
      },
    });
    ...
    assert.deepEqual(writes.map((event) => event.data), ['\r', '\r']);
    assert.ok(writes[0].t < writes[1].t, `writes out of order: ${JSON.stringify(writes)}`);
  });

  test('trusted cwd development-channel-only startup still receives one Enter', async () => {
    const result = await runWrapper({
      env: {
        CLAUDE_STDIN_AUTO_ACCEPT_EARLY_MS: '5000',
        PTY_MOCK_EXIT_MS: '400',
        PTY_MOCK_DATA_EVENTS: JSON.stringify([
          { delayMs: 10, data: developmentChannelPromptText() },
          { delayMs: 220, data: 'Listening for channel messages from: server:ipc' },
        ]),
      },
    });
  ...
  test('direct listening startup suppresses prompt and fallback writes', async () => {
    const result = await runWrapper({
      env: {
  ...
  test('large padded prompt streams persist markers across tail truncation', async () => {
    const result = await runWrapper({
      env: {
        CLAUDE_STDIN_AUTO_ACCEPT_EARLY_MS: '5000',
        PTY_MOCK_EXIT_MS: '800',
        PTY_MOCK_DATA_EVENTS: JSON.stringify([
          { delayMs: 10, data: `${'a'.repeat(5000)}Quick safety check: Is this a project you created or one you trust?` },
          { delayMs: 30, data: `${'b'.repeat(5000)}Claude Code'll be able to read, edit, and execute files here.` },
          { delayMs: 240, data: `${'c'.repeat(5000)}WARNING: Loading development channels` },
          { delayMs: 260, data: `${'d'.repeat(5000)}Channels: server:ipc` },
          { delayMs: 280, data: `${'e'.repeat(5000)}I am using this for local development` },
          { delayMs: 300, data: `${'f'.repeat(5000)}Enter to confirm` },
        ]),
      },
    });

    assert.equal(result.code, 0);
    assert.deepEqual(writeEvents(result).map((event) => event.data), ['\r', '\r']);
  });

  test('fallback still writes one Enter when no prompt and no listening marker appears', async () => {
    const result = await runWrapper({
      env: {
        CLAUDE_STDIN_AUTO_ACCEPT_EARLY_MS: '50',
        PTY_MOCK_EXIT_MS: '250',
        PTY_MOCK_DATA_EVENTS: JSON.stringify([]),
      },
    });
