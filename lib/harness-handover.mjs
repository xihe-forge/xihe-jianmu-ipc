...
export const DEFAULT_CHECKPOINT_PATH = 'D:/workspace/ai/research/xiheAi/xihe-tianshu-harness/.session-checkpoint.md';
export const DEFAULT_STATUS_PATH = 'D:/workspace/ai/research/xiheAi/xihe-company-brain/portfolio/STATUS.md';
export const DEFAULT_HANDOVER_REPO_PATH = 'D:/workspace/ai/research/xiheAi/xihe-tianshu-harness';
export const DEFAULT_LAST_BREATH_PATH = join(os.homedir(), '.claude', 'session-state', 'harness.json');
...
function defaultLastBreathPath() {
  return DEFAULT_LAST_BREATH_PATH;
}
...
async function runGit(cwd, args) {
  const { stdout } = await execFileAsync(await resolveGitExecutable(), args, { cwd, timeout: DEFAULT_GIT_TIMEOUT_MS });
  return stdout.trim();
}
...
async function allocateHandoverPath(outputDir, baseName, { dryRun = false } = {}) {
  if (dryRun && !await pathExists(outputDir)) {
    return join(outputDir, `${baseName}.md`);
  }
...
function toShortCommitSha(value) {
...
function normalizeInFlightTaskText(value) {
  return String(value ?? '')
    .replace(/^\[\d{1,2}:\d{2}\]\s*/, '')
    .replace(/^\*\*?in-flight\*\*?[：:]\s*/i, '')
    .replace(/^in-flight[：:]\s*/i, '')
    .trim();
}
...
function buildInFlightTask({ checkpoint, statusContent }) {
  const statusOverview = splitSectionLines(extractMarkdownSection(statusContent, '一眼看'));
  const candidate = [
    ...checkpoint.doing,
    ...checkpoint.nextSteps,
    ...statusOverview,
  ]
    .map(normalizeInFlightTaskText)
    .find(Boolean);

  return candidate || '先恢复上下文并确认当前最紧要的协调任务';
}

function buildNextSteps({ checkpoint, statusContent, handoverRelativePath, commitSha }) {
  const commitLabel = toShortCommitSha(commitSha) ?? 'local';
  const inFlightTask = buildInFlightTask({ checkpoint, statusContent });
  return [
    `1. 读本文件（${handoverRelativePath}）@ commit ${commitLabel}`,
    '2. 按 session-cold-start.md v1.0 的 7 步清单冷启（date / state.json / ipc_whoami / ipc_recent_messages / STATE.md / 冷启继承 IPC / critique落盘）',
    `3. 续前任 in-flight task：${inFlightTask}`,
  ];
}
...
function buildRunCheckPayload({ createdAtIso, handoverFile, commitSha }) {
  return {
    to: 'tech-worker',
    topic: 'run-check-sh',
    content:
      `【jianmu-pm ${createdAtIso} · run-check-sh】\n`
      + `commit: ${commitSha ?? 'dry-run'}\n`
      + `path: ${handoverFile.replace(/\\/g, '/')}\n`
      + 'mode: --only HANDOVER',
  };
}
...
export async function triggerHarnessSelfHandover({
  ...
  const resolvedOutputDir = dryRun
    ? join(handoverRepoPath, HANDOVER_DIRNAME)
    : (outputDir ?? join(handoverRepoPath, HANDOVER_DIRNAME));
  if (!dryRun) {
  ...
  const handoverFilename = basename(handoverFilePath);
  ...
  if (dryRun) {
    return {
      ...
      handoverFilename,
      handoverRelativePath,
      handoverContent: handoverMarkdown,
      commitSha: null,
      spawnResult: null,
      lineageDepth,
      handoverReason,
  ...
  await execFileAsync(await resolveGitExecutable(), [
    ...
    '-m',
    commitMessage,
  ], { cwd: handoverRepoPath, timeout: DEFAULT_GIT_TIMEOUT_MS });
  ...
  if (typeof ipcSend === 'function') {
    const runCheckPayload = buildRunCheckPayload({
      createdAtIso,
      handoverFile: handoverFilePath,
      commitSha,
    });
    ipcMessages.push(runCheckPayload);
    await ipcSend(runCheckPayload);
  }
  try {
