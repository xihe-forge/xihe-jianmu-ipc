...
export function analyzeTranscript(session, options = {}) {
  ...
  entries.forEach((entry, index) => {
    ...
    if (entry.type === 'user' || entry.message?.role === 'user') {
      lastUserIndex = index;
      lastUserText = contentToText(entry.message?.content);
      lastUserAt = ts;
    }
  });
  ...
  for (const entry of afterUser) {
    const ts = toTimestamp(entry.timestamp) ?? 0;
    if (hasAnyToolUse(entry)) lastToolUseAt = Math.max(lastToolUseAt, ts);
...
function countRecentCommits(cwd, sinceMs, execFileSyncImpl = execFileSync) {
  if (typeof cwd !== 'string' || cwd.trim() === '' || !existsSync(cwd)) return { count: 0, lastAt: 0 };
  try {
    const stdout = execFileSyncImpl('git', [
      'log',
...
function hasCodexProcessForCwd(session, options = {}) {
  if (typeof options.hasCodexProcessForCwd === 'function') {
    return options.hasCodexProcessForCwd(session);
  }
  return false;
}
...
async function fetchOutboundMessages(sessionName, { fetchImpl, ipcPort, hubAuthToken, sinceMs }) {
  ...
  const result = await fetchJson(fetchImpl, `http://127.0.0.1:${ipcPort}/messages?${params.toString()}`, { headers });
...
function newestActionAt({ transcript, fileMtimeAt, commitLastAt, outboundLastAt }) {
  return Math.max(
...
export async function evaluateIdlePatrolSession(session, options = {}) {
  ...
  if (!Number.isFinite(connectedAt) || (session.connected === false && nowMs - connectedAt >= options.offlineGraceMs)) {
    return { session: sessionName, action: 'skip', reason: 'offline' };
  }
  ...
  if (hasCodexProcessForCwd(session, options)) return { session: sessionName, action: 'skip', reason: 'codex-bg' };
  ...
  const fileMtimeAt = typeof options.findRecentMtime === 'function'
    ? options.findRecentMtime(session.cwd, sinceMs, options)
    : 0;
  const commits = options.countRecentCommits(session.cwd, sinceMs, options.execFileSync);
  ...
  if (nowMs - actionAt < options.idleThresholdMs) {
  ...
  const idleMinutes = Math.max(0, Math.floor((nowMs - actionAt) / 60_000));
