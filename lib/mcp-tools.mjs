...
// 9 个 MCP 工具定义。保持与现有行为和 schema 一致。
export const MCP_TOOL_DEFINITIONS = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        host: {
          ...
          default: 'external',
          description: 'Spawn host: wt=Windows Terminal new tab / vscode-terminal=VSCode terminal / external=caller handles',
        },
        cwd: {
          type: 'string',
          description: 'Working directory for the spawned session. Defaults to caller process.cwd() for backward compatibility.',
        },
      },
      required: ['name', 'task'],
    },
  ...
  },
  {
    ...
    description: 'Retrieve recent messages addressed to this session (or broadcast), persisted even if the session was offline or crashed. Use on cold-start to recover backlog from the dead period.',
    inputSchema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: 'Session name to query backlog for. Omit to use current session.' },
        since: { type: 'number', description: 'Milliseconds ago to look back. Default 6h (21600000), max 7d.' },
  ...
  async function handleToolCall(name, args) {
    ...
    if (name === 'ipc_spawn') {
      ...
      if (!isValidSessionName(sessionName)) {
        return toTextResult(`Invalid session name "${sessionName}": only letters, numbers, underscore and hyphen allowed`, true);
      }

      if (host !== undefined && !isValidSpawnHost(host)) {
        return toTextResult(`Invalid host "${host}": must be one of wt, vscode-terminal, external`, true);
      }

      try {
        const sessions = await httpGet(`http://${getHubHost()}:${getHubPort()}/sessions`);
        const existing = Array.isArray(sessions) && sessions.find((session) => session.name === sessionName);
        if (existing) {
          return toTextResult(`Session "${sessionName}" is already online. Use a different name or wait for it to disconnect.`, true);
        }
      } catch {
    ...
    if (name === 'ipc_rename') {
      ...
      if (!isValidSessionName(newName)) {
        return toTextResult(`Invalid session name "${newName}": only letters, numbers, underscore and hyphen allowed`, true);
      }
    ...
    if (name === 'ipc_task') {
      ...
      if (action === 'list') {
        try {
          ...
          const result = await httpGet(`http://${getHubHost()}:${getHubPort()}/tasks?${params.toString()}`);
          return toJsonResult(result);
        } catch (err) {
    ...
    if (name === 'ipc_recent_messages') {
      ...
      try {
        ...
        const result = await httpGet(`http://${getHubHost()}:${getHubPort()}/recent-messages?${params.toString()}`);
        return toJsonResult({
