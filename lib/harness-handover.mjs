...
function lowercaseCompatFilename(nowValue) {
  const parts = formatDateParts(nowValue);
  return `handover-${parts.yyyy}${parts.mm}${parts.dd}-harness.md`;
}
...
function buildRunCheckPayload({ createdAtIso, handoverFile, commitSha, dryRun }) {
  return {
    to: dryRun ? 'tech-worker-stub' : 'tech-worker',
    topic: 'run-check-sh',
    content:
...
export async function triggerHarnessSelfHandover({
  ...
  let compatAlias = null;
  if (dryRun) {
    compatAlias = join(outputDir, lowercaseCompatFilename(nowValue));
    await writeFile(compatAlias, handoverMarkdown, 'utf8');
  }
  ...
  if (typeof ipcSend === 'function') {
    const runCheckPayload = buildRunCheckPayload({
      createdAtIso,
      handoverFile: compatAlias ?? handoverFilePath,
      commitSha,
      dryRun,
    });
