...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_session_instances_last_seen ON session_instances(last_seen);
`);
...
`);
const getSessionInstanceStmt = db.prepare(`
  SELECT name, instance_id, first_seen, last_seen
  FROM session_instances
  WHERE name = @name AND instance_id = @instanceId
`);
const upsertSessionInstanceStmt = db.prepare(`
  INSERT INTO session_instances (name, instance_id, first_seen, last_seen)
  VALUES (@name, @instanceId, @firstSeen, @lastSeen)
...
export function hasSeenSessionInstance(name, instanceId) {
  if (typeof name !== 'string' || name.length === 0) return false;
  if (typeof instanceId !== 'string' || instanceId.length === 0) return false;
  return Boolean(getSessionInstanceStmt.get({ name, instanceId }));
}

export function rememberSessionInstance(name, instanceId, now = Date.now()) {
  if (typeof name !== 'string' || name.length === 0) return 0;
  if (typeof instanceId !== 'string' || instanceId.length === 0) return 0;
  return upsertSessionInstanceStmt.run({
    name,
    instanceId,
    firstSeen: now,
    lastSeen: now,
