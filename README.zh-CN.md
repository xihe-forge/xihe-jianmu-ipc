...
- `ipc_cost_summary(window?, group_by?, granularity?)`：ADR-013 ccusage 成本聚合查询；`window=today|7d|30d|all`，`group_by=none|ipc_name|model`，`granularity=hour|day`；`today` 默认小时桶，`hour + ipc_name` 返回每对话 × 24 小时矩阵
