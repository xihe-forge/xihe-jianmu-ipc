...
export function createHttpHandler(ctx) {
  ...
  function getFailingHash(failing = []) {
    const normalized = Array.isArray(failing)
      ? failing.map((name) => String(name)).sort()
      : [];
  ...
  function validateInternalEventPayload(payload) {
    ...
    if (payload.event === 'network-down') {
      if (!Array.isArray(payload.failing) || payload.failing.length === 0 || payload.failing.some((name) => typeof name !== 'string')) {
  ...
  return function handleRequest(req, res) {
      ...
      req.on('end', async () => {
        ...
        try {
          if (payload.event === 'network-down') {
            ...
            stderr(`[ipc-hub] HTTP POST /internal/network-event: network-down broadcasted to ${result.broadcastTo} subscriber(s)`);
            res.writeHead(200, { 'Content-Type': 'application/json' });
      ...
      req.on('end', () => {
        ...
        try {
          createPendingRebind({
            ...
            releasedAt,
            ttlSeconds,
            nextSessionHint: typeof payload?.next_session_hint === 'string'
        ...
        audit('prepare_rebind', {
          ...
          topics,
          next_session_hint: typeof payload?.next_session_hint === 'string'
            ? payload.next_session_hint
            : null,
        });
    ...
    // ---- POST /suspend ----
      ...
      req.on('end', () => {
        ...
        const suspended = suspendSession({
          ...
          task_description: typeof payload?.task_description === 'string' ? payload.task_description : null,
    ...
    // ---- GET /recent-messages ----
      ...
      const since = clampRecentSince(Number(url.searchParams.get('since') ?? DEFAULT_RECENT_SINCE_MS));
